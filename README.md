...
## 🔗 [View deployment](https://sungbinlee.github.io/TicTacToeWithGPT/)
## 💡 Overview
...
## 🎞️ Review
이 프로젝트를 진행하면서 처음으로 깃허브의 tag와 release 기능을 사용해보았습니다. 기능 추가, 버그 수정 등의 작업을 완료한 후에 태그를 생성하고, 해당 버전의 변경사항을 상세하게 작성하는 방식으로 진행했습니다. 이전에는 이러한 기능들의 중요성과 활용 방법에 대해 제대로 인지하지 못했었는데, 이번 기회를 통해 그 가치를 깨닫게 되었습니다. 태그를 통해 각 버전의 식별과 추적이 용이해지고, 릴리즈를 통해 업데이트 내용과 변경사항을 명확하게 전달할 수 있고, 개발팀 내에서도 협업과 이해를 쉽게 할 수 있을것이라 생각 됩니다.

제가 아쉬웠던 점은 릴리즈 기능을 너무 남발했다는 것입니다. 테스트와 QA에 충분한 시간을 할애하지 못해 버그와 기능 동작의 불안정성이 증가했고, 릴리즈 기능의 가치가 상대적으로 하락했습니다. 실서비스 환경이였다면 사용자들은 새로운 업데이트에 집중하기 어려웠을것이며, 자주 변화하는 기능과 인터페이스로 인해 혼동과 불만을 겪었을 것입니다. 그리고 만일 팀프로젝트에서 릴리즈 주기없이 독립적으로 작업했다면 팀원들과의 협업과 커뮤니케이션이 더욱 어려워졌을 것 입니다. 또한 릴리즈를 너무 자주 하다보니 버전 관리가 복잡해지고 혼돈스러워졌습니다. 

- 위의 경험을 토대로 앞으로는 릴리즈 주기를 명확히 정의하고 일정한 주기로 릴리즈하는 것이 중요하다고 느꼈습니다. 
- 깃허브의 tag와 release 기능을 통해 프로젝트 관리와 협업시의 효율성이 크게 향상이 될 것입니다.

이러한 기능들을 앞으로도 올바르게 활용하여 프로젝트의 품질과 개발 과정의 효율성을 높이고자 합니다.
